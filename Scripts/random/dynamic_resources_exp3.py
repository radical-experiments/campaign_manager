...
if __name__ == "__main__":
    ...
    for cm_size in campaign_sizes:
        ...
        for _ in range(repetitions):
            planner = RandomPlanner(campaign=campaign, resources=resources, num_oper=num_oper, sid='heft_exp')
            tic = time()
            plan = planner.plan()
            toc = time()
            makespan, reactive, expected = get_makespan(plan, 4, 0, dynamic_res=True)
            results.loc[len(results)] = [cm_size, 'RANDOM', plan, makespan, reactive, expected, makespan - expected, reactive - expected, time]
            del planner

    results.to_csv('../../Data/random/StHeteroCampaigns_4DynHeteroResourcesRAND2.csv', index=False)
