...
if __name__ == "__main__":
    ...
    for cm_size in campaign_sizes:
        ...
        for _ in range(repetitions):
            planner = RandomPlanner(campaign=campaign, resources=resources, num_oper=num_oper, sid='random')
            tic = time()
            plan = planner.plan()
            toc = time()
    ...
    results.to_csv('../../Data/random/StHeteroCampaigns_4St100HeteroResourcesRAND2.csv', index=False)
