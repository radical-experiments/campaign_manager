...
if __name__ == "__main__":
    ...
    for res_num in num_resources:
        ...
        for _ in range(repetitions):
            planner = RandomPlanner(campaign=campaign, resources=resources, num_oper=num_oper, sid='test1')
            tic = time()
            plan = planner.plan()
            toc = time()
    ...
    results.to_csv('../../Data/random/DynFixedHomoResources_StHHeteroCampaignsRAND.csv', index=False)
